...
describe("health & capacity", () => {
  ...
  test("C3 android full does not affect ios (independent caps)", async () => {
    ...
    await cli(s.server, ["session", "create", "--no-wait", "--template", "pixel6-api35"]);
    await cli(s.server, ["session", "create", "--no-wait", "--template", "pixel6-api35"]); // queued
    const r = await cli(s.server, ["capacity"]);
...
describe("session lifecycle & queue", () => {
  ...
  test("S4 wait on active returns immediately", async () => {
    const created = await cli(s.server, ["session", "create", "--no-wait", "--template", "pixel6-api35"]);
  ...
  test("S5 wait on queued blocks then activates", async () => {
    await cli(s.server, ["session", "create", "--no-wait", "--template", "pixel6-api35"]);
    const occupier = await cli(s.server, ["session", "create", "--no-wait", "--template", "pixel6-api35"]);
  ...
  test("S7 releasing queued shifts positions", async () => {
    await cli(s.server, ["session", "create", "--no-wait", "--template", "pixel6-api35"]);
    await cli(s.server, ["session", "create", "--no-wait", "--template", "pixel6-api35"]);
    const x = await cli(s.server, ["session", "create", "--no-wait", "--template", "pixel6-api35"]);
    const y = await cli(s.server, ["session", "create", "--no-wait", "--template", "pixel6-api35"]);
  ...
  test("S12 unknown template → template_not_found", async () => {
    const r = await cli(s.server, ["session", "create", "--no-wait", "--template", "nope"]);
  ...
  test("S15 concurrency: 20 creates against max 5 → exactly 5 active", async () => {
    ...
    const results = await Promise.all(
      Array.from({ length: 20 }, () =>
        cli(s.server, ["session", "create", "--no-wait", "--template", "pixel6-api35"]),
      ),
    );
